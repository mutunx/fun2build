        ...
        for (const pId of idsWhichNeedToGetComment) {
            ...
            for (const rawItem of rawItems) {
                ...
                while (!matchName.done) {
                    ...
                    rawItem.content = rawItem.content.replaceAll("</a>",'');
                    matchName = matchReplyName.next();
                }
                logger.info(`|-> match reply `+replyIds.join(','))
                const comment = {
                    id:rawItem.guid,
                    pill_id:pId,
                    author:rawItem.author,
                ...
                // 更新文章内容
                logger.debug(`|-> pId:${pId} descript length:${rssItem.description.length}`)
                await prisma.pill.update({
                    where: {
                        id:pId
                    },
                    data: {
                        content:rssItem.description.replace('- Made with love by RSSHub(https://github.com/DIYgod/RSSHub)',''),
                    }
                })
